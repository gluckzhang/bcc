        ...
        prog = self._get_heading() + """
{
        ...
        prog = prog % (self.pid, self._get_if_top(), self.prep, self._get_entry_logic())
        return prog
        ...
        prog = self._get_heading() + """
{
        ...
        prog = prog % (self.pid, self._get_exit_logic(), self._get_if_top(), self._get_err())
...
""" % self.length
    ...
    def _generate_program(self):
        ...
        self.program += "BPF_PERF_OUTPUT(events);\n" # debug usage
    ...
    def _print_debug_info(self, cpu, data, size):
        event = self.bpf["events"].event(data)
        print(event.pid)

    def _main_loop(self):
        self.bpf["events"].open_perf_buffer(self._print_debug_info)
        while True:
            try:
